// Jquery Reel customized by Marcelo Piñeiro - 05/03/15
// ====================================================
$(document).ready(function(){

	$(this).find('.viewer').each(function(index){
		elemwidth = $(this).find('img').attr('width');
		elemheight = $(this).find('img').attr('height');
		$(this).closest('.viewer').css({ width: elemwidth, height: elemheight });
		content = '<div class="boxLoading"><div class="litleBox"><span></span><span></span><span></span><span></span><span></span><span></span></div></div><div class="botPLAY paused">►</div><div class="botMAS active">+</div><div class="botMENOS">-</div><div class="handlerDragX"><div class="line"></div><div class="handler"></div></div><div class="handlerDragY"><div class="line"></div><div class="handler"></div></div>';
		$(content).prependTo( $(this) );
	});

	$('.viewer').on('click', '.botPLAY.paused', function(){
		$(this).addClass('running');
		$(this).removeClass('paused');
		$(this).text('■');
		imgID = $(this).closest('.viewer').find('img.reel').attr('id');
		$(this).closest('.viewer').find('img#'+imgID).trigger('play', .5);
	});
	$('.viewer').on('click', '.botPLAY.running', function(){
		$(this).removeClass('running');
		$(this).addClass('paused');
		$(this).text('►');
		imgID = $(this).closest('.viewer').find('img.reel').attr('id');
		$(this).closest('.viewer').find('img#'+imgID).trigger('stop');
	});


	$('.viewer').on('click', '.botMAS.active', function(){
	  $(this).removeClass('active').unbind('click');
	  $(this).closest('.viewer').find('.botMENOS').addClass('active');

	  dropHandlers( $(this).closest('.viewer'), 'show');

	  ancho = $(this).closest('.viewer').width();
	  alto = $(this).closest('.viewer').height();
	  $(this).closest('.viewer').find('img').animate({ 
	  	width: ancho*2, 
	  	height: alto*2, 
	  	left: '-='+ancho/2+'', 
	  	top: '-='+alto/2+'' }, 400, function(){
	  		$(this).closest('.viewer').find('.boxLoading').css('z-index','10');
	  		elemLength = $(this).closest('.viewer').find('.boxLoading span').length;
	  		$(this).closest('.viewer').find('.boxLoading span').each(function(i){
	  			$(this).delay(i*64).animate({ opacity: 1 }
	  				,128
	  				,function(){ 
	  					$(this).animate({ opacity: 0 },128);
	  					if( i == elemLength -1 ){ $(this).closest('.boxLoading').css('z-index','0') }
	  				}
	  			);
	  		});
	    	$(this).reel("images", $(this).attr('data-zoom'));
	  });
	});

	$('.viewer').on('click', '.botMENOS.active', function(){
		console.log('.viewer CLICK!!');
	  $(this).removeClass('active').unbind('click');
	  $(this).closest('.viewer').find('.botMAS').addClass('active');

	  dropHandlers( $(this).closest('.viewer'), 'hide');

	  ancho = $(this).closest('.viewer').width();
	  alto = $(this).closest('.viewer').height();
	  $(this).closest('.viewer').find('img')/*.css({ transform: 'none' })*/.animate({
	  	width: ancho, 
	  	height: alto, 
	  	left: '0', 
	  	top: '0' }, 400, function(){
	    $(this).reel("images", $(this).attr('data-images'));
	  });
	});
	...
	var dropHandlers = function( elem, param ){

	  if( param == 'show' ){

	    elem.find('.handlerDragX').delay(500).animate({ top: 0, opacity: 1 }, 500);
	    elem.find('.handlerDragY').delay(500).animate({ right: 0, opacity: 1 }, 500);

	    elem.find('.handlerDragX .handler').draggable({
	      axis: 'x', 
	      containment: 'parent',
	      drag: function( event, ui ) {
	        count_x = ( ui.helper.parent().find('.line').width() / 2 ) - ui.position.left;
	        updatePosX( $(this), count_x );
	      }
	    });
	    elem.find('.handlerDragY .handler').draggable({
	      axis: 'y', 
	      containment: 'parent',
	      drag: function( event, ui ) {
	        count_y = ( ui.helper.parent().find('.line').height() / 2 ) - ui.position.top;
	        updatePosY( $(this), count_y );
	      }
	    });

	  }else if( param == 'hide' ){
	    elem.find('.handlerDragX .handler').draggable('destroy').css({ left: '48%' });
	    elem.find('.handlerDragY .handler').draggable('destroy').css({ top: '48%' });
	    elem.find('.handlerDragX').delay(500).animate({ top: '-20px', opacity: 0 }, 500);
	    elem.find('.handlerDragY').delay(500).animate({ right: '-20px', opacity: 0 }, 500);
	  }
	}

	var updatePosX = function( elem, pos ){

		pos2 = ($(elem).closest('.viewer').find('img').width()/2);
		$(elem).closest('.viewer').find('img').css({ left: pos - pos2/2 });
	   //$(elem).closest('.viewer').find('img').css({ transform: 'translateX('+pos+'px)' });
	}
	var updatePosY = function( elem, pos ){
		pos2 = ($(elem).closest('.viewer').find('img').height()/2);
		$(elem).closest('.viewer').find('img').css({ top: pos - pos2/2 });
	   //$(elem).closest('.viewer').find('img').css({ transform: 'translateY('+pos+'px)' });
	}
});
/*
